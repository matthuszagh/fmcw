        ...
        if j == channels*samples_in_sweep*2:
            ...
            if restart == False and previous_n != None:
                if n != (previous_n+1) & 0xff:
                    print('Lost a sweep. Previous {}, now {} at {}'.format(
                        previous_n, n, f.tell()))
                    restart = True
            if restart:
                find_start(f)
...
if 0:
    ...
    ch1[plot_i] += fir_gain * \
        2**(adc_bits-1)*np.sin(2*np.pi*50e3 *
                               np.linspace(0, tsweep, len(ch1[0])))
    ...
    for plot_j in range(moving_average-1, len(ch1)):
        ...
        if 0:
            ...
            imgplot = ax.pcolormesh(
                angles_masked*np.pi/180, d, fxdb.transpose())
        elif 0:
...
if 1:
    ...
    for e in xrange(len(sweeps)):
        ...
        fy = 20 * \
            np.log10(
                (adc_ref/(2**(adc_bits-1)*fir_gain*max_range_index))*np.abs(fy))
