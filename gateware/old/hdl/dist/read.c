...
static int readCallback(uint8_t *buffer, int length, FTDIProgressInfo *progress, void *userdata)
{
		...
		if (check) {
			...
			for (i = offset; i < length - 16; i += 16) {
				...
				if (start && (num != start + 0x4000)) {
					...
					fprintf(stderr,
						"Skip %7d blocks from 0x%08x to 0x%08x at blocks "
						"%10llu\n",
						delta, start - 0x4000, num,
	...
	if (progress) {
		fprintf(stderr,
			"%10.02fs total time %9.3f MiB captured %7.1f kB/s curr rate %7.1f kB/s "
			"totalrate %d dropouts\n",
			progress->totalTime, progress->current.totalBytes / (1024.0 * 1024.0),
			progress->currentRate / 1024.0, progress->totalRate / 1024.0, n_err);
	}
...
int main(int argc, char **argv)
{
	...
	if (check && outfile) {
		...
		fprintf(stderr, "%d errors of %llu blocks (%Le), %d (%Le) blocks skipped\n", n_err,
			(unsigned long long)blocks, (long double)n_err / (long double)blocks, skips,
			(long double)skips / (long double)blocks);
	exit(0);
}
	...
	if (strcmp(descstring, "FT2232HTEST") == 0) {
		...
		while (fgets(pb, 1023, outputFile) != NULL) {
			...
			if (sscanf(pa, "%6u%94s%6u", &num_start, bufr, &num_end) != 3) {
				fprintf(stdout, "Format doesn't match at line %8d \"%s", num_lines,
					pa);
				err_count++;
				line_num = num_save + 2;
			} else {
		...
		if (err_count)
			fprintf(stdout, "\n%d errors of %d data sets %f\n", err_count, num_lines,
				(double)err_count / (double)num_lines);
		else
