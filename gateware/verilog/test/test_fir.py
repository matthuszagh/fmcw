...
class FIRTB:
    ...
    async def reset(self):
        ...
        await RisingEdge(self.dut.clk)
        self.dut.arst_n <= 1
        # await RisingEdge(self.dut.srst_n)
        # await FallingEdge(self.dut.clk_pos_en)
    ...
    def gen_outputs(self):
        ...
        # out_pre_dec = np.convolve(self.inputs, self.taps, mode="same")
...
async def check_sequence(dut):
    ...
    diffs = []
    while i > 0:
        await FallingEdge(tb.dut.clk_pos_en)
        await ReadOnly()
        if tb.dut.dvalid.value.integer == 1:
...
#         await RisingEdge(tb.dut.clk_pos_en)
#         await ReadOnly()
#         bank_output_vars = [
#             tb.dut.bank0.dout,
#             tb.dut.bank1.dout,
