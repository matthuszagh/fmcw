...
We can redisplay this menu at any point by typing ~fmcw > help~ (~fmcw
> ~ is prompted as part of the shell display, so you only need to type
~help~), and we can exit with ~fmcw > exit~.
...
type ~fmcw > c~ (~conf~ is the only menu command that begins with the
letter ~c~).
