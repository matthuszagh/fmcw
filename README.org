#+caption: Real-time range detection performed by radar. Distance
#+caption: (y-axis) is in meters and the time (x-axis) is in
#+caption: seconds. Magnitude is in dBFS relative to the max ADC
#+caption: input.
