...
\draw (coupler.2) --++(0.5,0) to[short,-o] ++(0,-1) node[below]{to receiver};
...
detector, lowpass filter, amplifier and VCO (our VCO is an external
component). It looks like this (note that this diagram, along with the
phase detector and VCO diagrams are adapted from AoE).

\begin{circuitikz}[color=fgcolor]
\draw[->] (0,0) node[left]{$f_{\mathit{REF}}$} to[short,o-] ++(1,0)
  node[fourport,anchor=4,t=$\phi$-detector](phi){};
\draw[->] ($(phi.3)!0.5!(phi.2)$) --++(1,0)
  node[lowpassshape,anchor=w](lp){};
\draw[->] (lp.e) --++(1,0) node[ampshape,anchor=w](amp){};
\gettikzxy{(0,0)}{\finx}{\finy}
\draw (amp.e) --++(1,0) --++(0,-1.5) coordinate(c1);
\gettikzxy{(c1)}{\cx}{\cy}
\gettikzxy{(phi.1)}{\phix}{\phiy}
\draw[->] (c1) --++({(\finx-\cx)/2},0) coordinate(c3);
\draw (c3) node[vcoshape,anchor=e](vco){};
\gettikzxy{(vco.e)}{\vcoex}{\vcoey}
\gettikzxy{(vco.w)}{\vcowx}{\vcowy}
\draw (vco.w) --++({(\finx-\cx)/2+(\vcoex-\vcowx)},0)
  --++(0,\phiy-\cy) coordinate(c2);
\draw[->] (c2) -- (phi.1);
\draw (c2) node[left]{$f_{\mathit{VCO}}$};
\end{circuitikz}
...
input voltage. When the rising edge of \(f_{\mathit{REF}}\) occurs
before the rising edge of

\(f_{\mathit{in}}\) is a reference clock frequency (\(\SI{40}{MHz}\)
from a crystal oscillator in our case).
