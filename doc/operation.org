...
block diagram of this is shown below. I've omitted the ADC's second
input channel since it's identical to the first. Both channels are
time-multiplexed across the same 12-bit data bus to the FPGA.
...
** FT2232H
The FT2232H is a USB interface chip that permits 2-way communication
with the FPGA and enables us to load a bitstream onto the FPGA. The
attached EEPROM chip is required to use the FT2232H in its FT245
synchronous FIFO mode, which allows the full \(\SI{480}{Mbps}\) data
transfer speed.
