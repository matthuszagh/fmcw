...
\draw (adf.bpin 5) to[short] ++(1.5,0) node[inputarrow]{}
  \block(control){\footnotesize control};
%% ADC
...
\draw ($(fft.s)-(0.28,0)$) node[above]{\tiny Im}
  --++(0,-1) coordinate (fftIm);
\draw ($(fft.s)+(0.28,0)$) node[above]{\tiny Re}
  --++(0,-1) coordinate (fftRe);
...
\draw (rdfifo.bpin 2) --++(0.25,0) |-++(0.25,2) node[inputarrow]{}
  \block(controlb){\footnotesize control};
\draw (controlb.bpin 5) -| (largemux.tpin 1) node[left]{\tiny
  OP STATE};
...
\draw[dashed] (-1,-7) node[below right]{\footnotesize PC} -| (0.5,-11);
\end{circuitikz}
