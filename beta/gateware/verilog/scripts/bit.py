...
def num_bits_for_sint(val: int) -> int:
    """
    Minimum number of bits needed to represent a signed integer value
    using two's complement representation.
    """
    if val < 0:
        return int(np.ceil(np.log2(-val) + 1))
    else:
        return int(np.ceil(np.log2(val + 1) + 1))
...
def quantized_complex(cvalue, prec):
    """
    """
    return quantized_real(np.real(cvalue), prec) + 1j * quantized_real(
        np.imag(cvalue), prec
    )
